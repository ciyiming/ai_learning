...
#对二维矩阵来讲a[0][1]会有两个索引方向，默认按列方向搜索最大值
#axis=0按列方向搜索最大值,axis=1行方向搜索最大值

# 建立NN model，同时完成BP的loss回传，更新model参数
def build_model(nn_hdim, num_passes=30000, print_loss=False):

    # 权值初始化 W1为输入层到隐藏层的权值矩阵，W2为隐藏层到输出层的权值矩阵
    np.random.seed(0)
    W1 = np.random.randn(nn_input_dim, nn_hdim) / np.sqrt(nn_input_dim)  #randn函数根据给定维度生成[0,1)之间的数据，包含0，不包含1
    # 为什么要除以np.sqrt(nn_input_dim)？为了让方差为0
    ...
    for i in range(0, num_passes):
        ...
        a1 = np.tanh(z1)
        z2 = a1.dot(W2) + b2
        exp_scores = np.exp(z2)  # softmax第一步，换为以e为底的幂
        probs = exp_scores / np.sum(exp_scores, axis=1, keepdims=True)  # softmax第二步，除以所有值的e为底的幂的和
        #np.sum(a, axis=1, keepdims=True), axis=1按行相加，axis=0按列相加，没有axis参数矩阵每个元素相加
# keepdims=True保持矩阵的二维特性
        # 反向传播，计算loss对权值的梯度
        delta3 = probs
